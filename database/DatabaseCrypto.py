...
class DatabaseCrypto(Database):
    def __int__(self, market_name):
        super(DatabaseCrypto, self).__int__(market_name)
    ...
    def add_to_data_base(self, crypto_currency_symbol, fiat_symbol):
        ...
        for resolution in self.main_container[crypto_currency_symbol]:
            ...
            candle_objects = change_candles_to_candle_objects(candles_json, resolution, crypto_currency_symbol,
                                                              fiat_symbol)
            for candle in candle_objects:
                candle.counter = len(self.main_container[crypto_currency_symbol][resolution][fiat_symbol])
    ...
    def update_candles_on_currency(self, crypto_currency_symbol):
        ...
        if latest_date_dict is None or len(latest_date_dict) == 0:
            return
        # {'15': {'USDT': 1524386700}, '30': {'USDT': 1524835800}, '60': {'USDT': 1525734000}, 'D': {'USDT':
        # 1567036800}, 'W': {'USDT': 1632700800}, 'M': {'USDT': 1633046400}}
                ...
                while not finished:
                    if not is_comparable_with_current_time(latest_date_dict[resolution][fiat_currency], resolution):
                        ...
                        if candles_json is not None:
                            candle_objects = change_candles_to_candle_objects(candles_json, resolution,
                                                                              crypto_currency_symbol, fiat_currency)
                            for candle in candle_objects:
                                candle.counter = len(self.main_container[crypto_currency_symbol][resolution][fiat_currency])
                        ...
                        # It download current value candle on the stock. It is removed here to avoid any issues
